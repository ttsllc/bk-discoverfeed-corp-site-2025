...
require get_template_directory() . '/admin/inc/product.php';
require get_template_directory() . '/admin/inc/support.php';
...
/**
 * anthem オプションTools デフォルト画像設定フィルター
 */
function anthem_theme_options_tools_get_default_images_settings( $default_images_settings ) {
	// デフォルト画像設定
	return array(
/*
		array(
...
*/
		...
		array(
			'media_filename' => 'image_1450x900.gif',
			'source_filepath' => get_template_directory() . '/admin/img/default/image_1450x900.gif',
		...
		array(
			...
			'theme_option_keys' => array( "['contents_builder'][0]['bg_image']" )
		),
		array(
			'media_filename' => 'image_1450x500.gif',
			'source_filepath' => get_template_directory() . '/admin/img/default/image_1450x500.gif',
			'theme_option_keys' => array( 'blog_bg_image')
		),
		array(
			'media_filename' => 'image_1450x400.gif',
			'source_filepath' => get_template_directory() . '/admin/img/default/image_1450x400.gif',
			'theme_option_keys' => array( 'footer_bg_image' )
		)
	);
}
...
/**
 * anthem オプションTools サンプルカテゴリー
 */
function anthem_theme_options_tools_set_sample_categories( $dp_options ) {
	// 新規サイト以外、リセットチェックなしの場合は終了
	if ( ! theme_options_tools_is_new_site() && empty( $_POST['tcd-tools-reset-sample-categories'] ) )
		return $dp_options;

	// 商品カテゴリーのアイキャッチ画像
	$product_category_media_id = (int) theme_options_tools_get_media_id( array(
		'media_filename' => 'image_172x180.gif',
		'source_filepath' => get_template_directory() . '/admin/img/default/image_172x180.gif'
	), true );
	...
	for ( $i = 1; $i <= 7; $i++ ) {
		$sample_categories[] = array(
			...
			'slug' => 'product_category'.$i,
			'taxonomy' => 'product_category',
			'metas' => array(
				'sub_title' => __( 'Subtitle', 'tcd-w' ),
				'image' => $product_category_media_id
			)
		);
	}
	for ( $i = 1; $i <= 2; $i++ ) {
...
/**
 * anthem オプションTools サンプル記事 ---------------------------------------------------------------------------
 */
function anthem_theme_options_tools_set_sample_posts( $dp_options ) {

	// ブログのアイキャッチ画像
	$media_id = (int) theme_options_tools_get_media_id( array(
		'media_filename' => 'image_800x460.gif',
		'source_filepath' => get_template_directory() . '/admin/img/default/image_800x460.gif'
	), true );

	// 商品のカスタムフィールド用画像
	$product_custom_media_id = (int) theme_options_tools_get_media_id( array(
		'media_filename' => 'image_470x290.gif',
		'source_filepath' => get_template_directory() . '/admin/img/default/image_470x290.gif'
	), true );
	...
	for ( $i = 1; $i <= 7; $i++ ) {
		$sample_posts[] = array(
			'post_title' => __( 'Sample product', 'tcd-w' ) . $i,
			'post_content' => __( 'Sample text. Sample text.', 'tcd-w' ),
			'post_name' => 'sample-product' . $i,
			'post_status' => 'publish',
			'post_type' => 'product',
			'post_date' => $post_date_base . sprintf( '%02d', $i ),
			...
			'metas' => array(
				'carousel_image' => $product_custom_media_id,
				'short_desc' => __( 'Description will be displayed here.<br />Description will be displayed here.', 'tcd-w' )
			)
		);
	}
	...
	for ( $i = 1; $i <= 4; $i++ ) {
		$sample_posts[] = array(
			'post_title' => __( 'Sample support', 'tcd-w' ) . $i,
			'post_content' => __( 'Sample text. Sample text.', 'tcd-w' ),
			'post_name' => 'sample-support' . $i,
			'post_status' => 'publish',
			'post_type' => 'support',
			'post_date' => $post_date_base . sprintf( '%02d', $i ),
			'taxonomies' => array(
				'support_category' => array('support_category1','support_category2'),
			),
		);
	}
...
/**
 * anthem オプションTools サンプルメニュー ------------------------------------------------------------------
 */
function anthem_theme_options_tools_set_sample_menus( $dp_options ) {
	...
	$menu_items = array(
		...
		array(
			...
			'product_category' => array(
				array(
					'name' => __( 'Product category', 'tcd-w' ) . '1',
					'url' => '#',
					'type' => 'product_category'
				),
				array(
					'name' => __( 'Product category', 'tcd-w' ) . '2',
					'url' => '#',
					'type' => 'product_category'
				),
				array(
					'name' => __( 'Product category', 'tcd-w' ) . '3',
					'url' => '#',
					'type' => 'product_category'
				),
				array(
					'name' => __( 'Product category', 'tcd-w' ) . '4',
					'url' => '#',
					'type' => 'product_category'
				),
				array(
					'name' => __( 'Product category', 'tcd-w' ) . '5',
					'url' => '#',
					'type' => 'product_category'
				),
				array(
					'name' => __( 'Product category', 'tcd-w' ) . '6',
					'url' => '#',
					'type' => 'product_category'
				),
				array(
					'name' => __( 'Product category', 'tcd-w' ) . '7',
					'url' => '#',
					'type' => 'product_category'
				),
			)
	...
	foreach( $menu_items as $menu_item ) {
		...
		if ( $menu_item_db_id && ! is_wp_error( $menu_item_db_id ) ) {
			...
			// 子メニューアイテム作成　商品カテゴリー
			if ( ! empty( $menu_item['product_category'] ) && is_array( $menu_item['product_category'] ) ) {

				foreach ( $product_category_slugs as $product_category_slug ) {
					$term = get_term_by( 'slug', $product_category_slug, 'product_category' );
					if ( ! empty( $term->term_id ) ) {
						wp_update_nav_menu_item( $menu_id, 0, array(
							'menu-item-type' => 'taxonomy',
							'menu-item-title' => $term->name,
							'menu-item-url' => get_term_link($product_category_slug,'product_category'),
							'menu-item-object' => $term->taxonomy,
							'menu-item-object-id' => $term->term_id,
							'menu-item-parent-id' => $menu_item_db_id,
							'menu-item-status' => 'publish'
...
/**
 * anthem オプションTools サンプルウィジェット（ブログページ左） -----------------------------------------------------------------
 */
function anthem_theme_options_tools_set_sample_widgets( $dp_options ) {
	...
	$sample_widgets = array(
		...
		// カテゴリ―
		array(
			'class_name' => 'tcd_category_list_widget',
			'params' => array(
	...
	// ウィジェットエリアの設定
	foreach( array( 'blog_widget' ) as $sidebar ) {
		// 現ウィジェットを設定を削除
 ...
 * anthem テーマ初期化フィルター ----------------------------------------------------------------------------
 */
add_filter( 'tcd_theme_options_tools-get_default_images_settings', 'anthem_theme_options_tools_get_default_images_settings', 10, 1 );
add_filter( 'tcd_theme_options_tools-initialize', 'anthem_theme_options_tools_set_sample_categories', 10, 1 );
add_filter( 'tcd_theme_options_tools-initialize', 'anthem_theme_options_tools_set_sample_posts', 10, 1 );
add_filter( 'tcd_theme_options_tools-initialize', 'anthem_theme_options_tools_set_sample_menus', 10, 1 );
add_filter( 'tcd_theme_options_tools-initialize', 'anthem_theme_options_tools_set_sample_widgets', 10, 1 );
