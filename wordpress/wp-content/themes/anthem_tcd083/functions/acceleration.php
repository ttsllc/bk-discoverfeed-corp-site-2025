...
function tcd_acceleration_wp() {
	...
	// テーマオプション高速化チェック
	if ( ! empty( $dp_options['use_lazyload'] ) || ! empty( $dp_options['use_js_optimization'] ) || ! empty( $dp_options['use_css_optimization'] ) || ! empty( $dp_options['use_html_optimization'] ) ) {
		...
		// LazyLoad DOMDocumentクラス必須
		if ( ! empty( $dp_options['use_lazyload'] ) && class_exists( 'DOMDocument' ) ) {
